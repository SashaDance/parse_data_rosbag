...
bridge = CvBridge()
frames = list(np.random.choice(a=len(rows), size=20))
img_timestamps = []
os.mkdir('data')
...
for img_timestamp in img_timestamps:
    ...
    open3d.io.write_point_cloud(f'data/{img_timestamp}/point_cloud_{closet_timestamp[0]}_1.pcd', pcd)

    pc = pointcloud2_to_array(point_cloud_msg)
    points = np.stack((pc['x'], pc['y'], pc['z']), axis=-1).reshape(-1, 3)
    pcd = open3d.geometry.PointCloud()
    pcd.points = open3d.utility.Vector3dVector(points)
    open3d.io.write_point_cloud(f'data/{img_timestamp}/point_cloud_{closet_timestamp[0]}_2.pcd', pcd)
