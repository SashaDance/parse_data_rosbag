...
conn = sqlite3.connect('/media/sashadance/HIKVISION/rosbag2_2025_02_21-16_36_05/rosbag2_2025_02_21-16_36_05_11.db3')
cursor = conn.cursor()

cursor.execute("SELECT timestamp, data from messages where topic_id = 4")
rows = cursor.fetchall()

bridge = CvBridge()
frames = list(np.random.choice(a=len(rows), size=100, replace=False))
img_timestamps = []
os.mkdir('data')
...
cursor.execute("SELECT timestamp from messages where topic_id = 5")
rows = cursor.fetchall()
...
for img_timestamp in img_timestamps:
    ...
    cursor.execute(f"SELECT data from messages where topic_id = 5 and timestamp = {closet_timestamp[0]}")
    ...
    dict_stats[img_timestamp] = (img_timestamp - closet_timestamp[0]) / 1e9
    
with open('data_stats.pkl', 'wb') as f:
    pickle.dump(dict_stats, f)
